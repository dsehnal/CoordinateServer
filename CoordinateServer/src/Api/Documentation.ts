...
function createDocumentationHTML(appPrefix: string) {
    ...
    for (let entry of Queries.QueryList) {
        ...
        html.push(
            `<div>`,
            `<a href="${appPrefix}${url}" title="Fill in the desired values. Empty-string parameters are ignored by the server." class="cs-docs-template-link" target="_blank">${url}</a>`,
