...
function createDocumentationHTML(appPrefix) {
    ...
    for (var _i = 0, _a = Queries.QueryList; _i < _a.length; _i++) {
        ...
        if (params.length > 0) {
            ...
            for (var _b = 0, params_1 = params; _b < params_1.length; _b++) {
                ...
                if (p.defaultValue !== void 0) {
                    html.push("= <b title='Default value'>" + p.defaultValue + "</b>");
                }
                if (p.description) {
        ...
        html.push("<div>", "<a href=\"" + url + "\" title=\"Fill in the desired values. Empty-string parameters are ignored by the server.\" target=\"_blank\"><code>" + url + "</code></a>", "<div style='color: #424242; font-size: 85%; margin: 10px'>Fill in <i>PDBID</i> and other parameters to customize the query.<br/>", "Empty-string values of parameters are ignored by the server, e.g. <code>/entities?entityId=&type=water</code> is the same as <code>/entities?type=water</code>.</br>", "Names of residues/chains/entities/etc. are case sensitive.</div>", "</div>");
        html.push("</div>");
        html.push("</div>");
    }
    html.push("<i>Note:</i><br/>");
