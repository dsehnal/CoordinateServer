...
var fCifCategoryWriters;
(function (fCifCategoryWriters) {
    ...
    fCifCategoryWriters.CifWriterContents = CifWriterContents;
    ...
    fCifCategoryWriters.writeRecords = writeRecords;
    ...
    function writeHelices(content, writer) {
        ...
        var fields = [
            ...
            { name: '_struct_conf.start_residue_index', src: function (ctx, i) { return ctx.indices.starts[i].toString(); } },
            { name: '_struct_conf.end_residue_index', src: function (ctx, i) { return ctx.indices.ends[i].toString(); } },
    ...
    function writeSheets(content, writer) {
        ...
        var fields = [
            ...
            { name: '_struct_sheet_range.start_residue_index', src: function (ctx, i) { return ctx.indices.starts[i].toString(); } },
            { name: '_struct_sheet_range.end_residue_index', src: function (ctx, i) { return ctx.indices.ends[i].toString(); } },
    ...
    function writeModRes(content, writer) {
        ...
        var fields = [
            ...
            { name: '_pdbx_struct_mod_residue.residue_index', src: function (ctx, i) { return ctx.residues[i].toString(); } },
    ...
    fCifCategoryWriters.CategoryWriters = {
        '_entry': writeEntry,
        '_entity': writeEntity,
    ...
    var AtomSiteGroupPDBStringWriter = (function () {
        function AtomSiteGroupPDBStringWriter(w, col) {
            this.w = w;
            this.col = col;
        }
        AtomSiteGroupPDBStringWriter.prototype.write = function (aI, offset) { this.w.write(this.col[aI] === 'ATOM' ? 'A ' : 'H '); };
        return AtomSiteGroupPDBStringWriter;
    }());
    ...
    function writeAtomSitesProper(data, model, atoms, writer) {
        ...
        var writers = atomSiteCols.map(function (c, i) {
            switch (c) {
                ...
                case '_atom_site.residue_index': return new AtomSiteCellModelIntegerWriter(writer, model.atoms.residueIndex);
                default:
                    var idx = cols.indexOf(c);
                    if (idx < 0)
                        return undefined;
                    return new AtomSiteCellTokenWriter(writer, idx, source, tokens);
            }
    ...
    function writeAtomSites(models, first, writer) {
        if (!models.some(function (m) { return m.fragments.length > 0; }))
            return;
        var atomsCat = first.data.getCategory('_atom_site');
        writer.write('loop_');
        for (var _i = 0, atomSiteCols_1 = atomSiteCols; _i < atomSiteCols_1.length; _i++) {
            var col = atomSiteCols_1[_i];
            writer.write('\n' + col);
        }
    ...
    fCifCategoryWriters.writeAtomSites = writeAtomSites;
    function writeResidueSites(content, writer) {
        var ctx = { residues: content.model.residues, idx: content.fragment.residueIndices };
        var fields = [
            { name: '_residue_site.is_het', src: function (ctx, i) { return ctx.residues.isHet[ctx.idx[i]] ? 'Y' : 'N'; } },
            { name: '_residue_site.name', src: function (ctx, i) { return ctx.residues.name[ctx.idx[i]]; } },
            { name: '_residue_site.auth_name', src: function (ctx, i) { return ctx.residues.authName[ctx.idx[i]]; } },
            { name: '_residue_site.seq_number', src: function (ctx, i) { return ctx.residues.seqNumber[ctx.idx[i]].toString(); } },
            { name: '_residue_site.auth_seq_number', src: function (ctx, i) { return ctx.residues.authSeqNumber[ctx.idx[i]].toString(); } },
            { name: '_residue_site.ins_code', src: function (ctx, i) { return ctx.residues.insCode[ctx.idx[i]]; } },
            { name: '_residue_site.chain_index', src: function (ctx, i) { return ctx.residues.chainIndex[ctx.idx[i]].toString(); } },
        ];
        writeRecords(fields, ctx, content.fragment.residueIndices.length, writer);
        writer.write('#\n');
    }
    fCifCategoryWriters.writeResidueSites = writeResidueSites;
    function writeChainSites(content, writer) {
        var ctx = { chains: content.model.chains, idx: content.fragment.chainIndices };
        var fields = [
            { name: '_chain_site.name', src: function (ctx, i) { return ctx.chains.asymId[ctx.idx[i]]; } },
            { name: '_chain_site.auth_name', src: function (ctx, i) { return ctx.chains.authAsymId[ctx.idx[i]]; } },
            { name: '_chain_site.entity_index', src: function (ctx, i) { return ctx.chains.entityIndex[ctx.idx[i]].toString(); } },
            { name: '_chain_site.model_number', src: function (ctx, i) { return '1'; } },
        ];
        writeRecords(fields, ctx, content.fragment.chainIndices.length, writer);
        writer.write('#\n');
    }
    fCifCategoryWriters.writeChainSites = writeChainSites;
})(fCifCategoryWriters || (fCifCategoryWriters = {}));
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = fCifCategoryWriters;
