...
var AtomSiteWriter = (function () {
    function AtomSiteWriter(atoms) {
        this.atoms = atoms;
        this.serial = 1;
        this.wasHET = false;
    ...
    AtomSiteWriter.prototype.writeRecord = function (i, writer) {
        //Start		Width		End		Data Type		Field		CIF field
        //1		6		6		Record Name		ATOM/ HETATM		_atom_site.group_PDB
        var recordName = this.recordName.getString(i);
        if (!recordName)
            recordName = "ATOM";
        ...
        var name = this.name.getString(i);
        if (name.length < 3)
            writer.writePadRight(" " + name, 4);
        else
            writer.writePadLeft(name, 4);
        //21		1		21		Char		altLoc		_atom_site.auth_alt_id
        var altLoc = this.altLoc.getString(i);
        if (!altLoc)
            writer.write(" ");
        else if (altLoc.length > 1)
            writer.write(altLoc.substr(0, 1));
        else if (altLoc.length === 1)
            writer.write(altLoc);
        else
            writer.write(" ");
        ...
        var iCode = this.iCode.getString(i);
        if (!iCode)
            writer.write(" ");
        else if (iCode.length > 1)
            writer.write(iCode.substr(0, 1));
        else if (iCode.length === 1)
            writer.write(iCode);
        else
            writer.write(" ");
        ...
        if (this.occupancy.isUndefined(i))
            writer.whitespace(6);
        else
            writer.writePadLeft(this.occupancy.getFloat(i).toFixed(2), 6);
        //106		6		111		Real 6.2		tempFactor		_atom_site.B_iso_or_equiv
        if (this.tempFactor.isUndefined(i))
            writer.whitespace(6);
        else
            writer.writePadLeft(this.tempFactor.getFloat(i).toFixed(2), 6);
        ...
        if (this.charge.isUndefined(i))
            writer.whitespace(2);
        else
            writer.writePadRight(this.charge.getString(i), 2);
        writer.newline();
    };
    AtomSiteWriter.prototype.writeTer = function (i, writer) {
        //TER
    ...
    AtomSiteWriter.prototype.write = function (writer) {
        var previous = 0;
        var modelCol = this.atoms.getColumn("_atom_site.pdbx_PDB_model_num").index;
        var authAsymIdCol = this.atoms.getColumn("_atom_site.auth_asym_id").index;
        var recordTypeCol = this.atoms.getColumn("_atom_site.group_PDB").index;
        var currentModel = new StringWriter_1.default();
        var models = [currentModel];
        for (var i = 0, _b = this.atoms.rowCount; i < _b; i++) {
            try {
                if (!this.wasHET &&
                ...
                if (!this.atoms.areTokensEqual(this.atoms.getTokenIndex(previous, modelCol), this.atoms.getTokenIndex(i, modelCol))) {
                    if (!this.wasHET)
                        this.writeTer(i, currentModel);
                    currentModel = new StringWriter_1.default();
                    this.serial = 1;
        ...
        if (!this.wasHET)
            this.writeTer(previous, currentModel);
        if (models.length === 1) {
    ...
    return AtomSiteWriter;
}());
var WidePDBWriter = (function () {
    function WidePDBWriter(data) {
        this.data = data;
    }
    WidePDBWriter.prototype.writeCryst1 = function (writer) {
        var cell = this.data.getCategory("_cell");
        var symmetry = this.data.getCategory("_symmetry");
        if (!cell || !symmetry)
            return;
    ...
    WidePDBWriter.prototype.writeHeader = function (writer) {
        ...
        writer.newline();
        var cat = this.data.getCategory("_pdbx_database_PDB_obs_spr");
        if (!cat)
            return;
        var col = cat.getColumn("_pdbx_database_PDB_obs_spr.replace_pdb_id");
        if (!col)
            return;
        var ids = col.getString(0).split(' ');
        for (var i = 0, o = 0; i < ids.length; i++) {
            if (!ids[i].length)
                continue;
    ...
    WidePDBWriter.prototype.write = function () {
        var writer = new StringWriter_1.default();
        var sites = new AtomSiteWriter(this.data.getCategory("_atom_site"));
    ...
    return WidePDBWriter;
}());
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = WidePDBWriter;
